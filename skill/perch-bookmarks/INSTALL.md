# Installing `perch-bookmarks`

URL bookmarking with tags and favicons.
...
```bash
cp -r skill/perch-bookmarks ~/.openclaw/skills/
```
...
```bash
mkdir -p ~/.claude/skills && cp -r skill/perch-bookmarks ~/.claude/skills/
```
