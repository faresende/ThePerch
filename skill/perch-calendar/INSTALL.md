# Installing `perch-calendar`

Ingest calendar events into Supabase as ISO8601-timezoned records.
...
```bash
cp -r skill/perch-calendar ~/.openclaw/skills/
```
...
```bash
mkdir -p ~/.claude/skills && cp -r skill/perch-calendar ~/.claude/skills/
```
...
- A calendar source (icalBuddy on macOS, any ICS/CalDAV provider, or your own feed)
