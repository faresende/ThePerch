...
| `scripts/orders_ingest_catchup.py` | 30-min catchup safety net (re-scans recent inbox/Paper-Trail) |

> The previous monolithic `scripts/orders_autopilot_ingest_fastmail.py` was retired on 2026-04-23 — see `scripts/archive/README.md`. The catchup script is the canonical in-repo path. A real-time JMAP listener (which would shell out to `cli.js process-email` per email) lives outside this repo as part of an openclaw install; bundle one yourself or rely on the 30-min catchup.
...
In-repo path is the catchup script — register it as a 30-minute cron:
