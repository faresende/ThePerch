# Installing `perch-health`

Sleep, HRV, and body-metrics ingestion.
...
```bash
cp -r skill/perch-health ~/.openclaw/skills/
```
...
```bash
mkdir -p ~/.claude/skills && cp -r skill/perch-health ~/.claude/skills/
```
...
- A sleep/HRV source. Reference provider: Oura Ring. Adapters for other sources go in `providers/`.
