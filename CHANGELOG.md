...
- **R12 MEDIUM** `anon` role retained `arwdDxtm` (full DML+TRUNCATE+REFERENCES) on 22 RLS-enabled public tables. RLS denies anon for SELECT/INSERT/UPDATE/DELETE because no policy targets `anon`, but TRUNCATE bypasses RLS in Postgres, and REFERENCES can be used as a row-existence oracle. R10 only revoked anon on `bookmarks`; new migration `20260429900000_round12_revoke_anon_grants.sql` sweeps every public table. Defense-in-depth — doesn't change any working code path.
