...
After Round 11, the 5-round budget (R7–R11) is exhausted. Each round caught regressions introduced by the prior round plus pre-existing gaps. R11 found 2 CRITICAL items (TravelHomeCard cold-launch regression + migration ledger drift), confirming the budget should be event-triggered going forward, not calendar-driven.
